...
closed_e = FOREACH closed_d GENERATE id, number, assignment_group, business_service, configuration_item, approval, closed_dt, created_dt, duration, month, opened_dt, priority, severity, short_description, state, updated_dt, username, sys_load_date, GetMonth(closed_dt) AS closed_month:chararray, GetMonth(opened_dt) AS opened_month:chararray,  GetYear(opened_dt) AS opened_year:chararray, GetYear(closed_dt) AS closed_year:chararray, SUBSTRING((chararray)GetYear(opened_dt),2,4) AS half_open_year:chararray, SUBSTRING((chararray)GetYear(closed_dt),2,4) AS half_close_year:chararray, (SecondsBetween(closed_dt, opened_dt)/(3600*24)) AS incident_duration:long;
